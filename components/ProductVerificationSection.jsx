...
const ProductVerificationSection = () => {
  ...
  // Scroll-based step progress tracking
  useEffect(() => {
    const handleScroll = () => {
  ...
  const getMobileTransform = (step, isLeft = true) => {
    ...
    const rotateY =
      ...
      easedProgress * (isLeft ? (isMobile ? 45 : 90) : -(isMobile ? 45 : 90));

    const rotateX = (isMobile ? 10 : 20) - easedProgress * (isMobile ? 10 : 20);

    const rotateZ =
      (isMobile ? (isLeft ? -5 : 5) : isLeft ? -10 : 10) +
      easedProgress * (isLeft ? (isMobile ? 5 : 10) : -(isMobile ? 5 : 10));
